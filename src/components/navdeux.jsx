...
const Navbarr = () => {
  ...
  const navItems = [
    ...
    { id: 2, text: 'Q&A VAULT', path: '/qvault' },
  ...
  return (
    <div className="mt-[5px] flex items-center justify-between h-[40px] w-full lg:px-20 bg-[#FAF5F0]">
      {/* Logo Section */}
      ...
      <div className="hidden md:flex items-center justify-end w-full">
        <ul className="font-raleway flex space-x-10 lg:space-x-16 text-bblue font-bold text-[20px]">
          {navItems.map((item) => (
            ...
            >
              {item.text}
              {/* Adjust the underline effect */}
              {activePage === item.text && (
                <span className="absolute left-0 right-0 bottom-0 h-1 bg-bblue"></span>
              )}
            </li>
          ))}
        </ul>
